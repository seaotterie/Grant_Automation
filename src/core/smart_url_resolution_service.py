...
class URLCandidate:
    ...
    url: str
    source: str  # 'user_provided', '990_declared', 'gpt_predicted'
...
class SmartURLResolutionService:
    ...
    async def _collect_url_candidates(self, 
        ...
        # Source 3: GPT-predicted URLs (algorithmic prediction)
        gpt_urls = await self._get_gpt_predicted_urls(ein, organization_name)
        for url in gpt_urls:
            gpt_candidate = URLCandidate(
                url=self._normalize_url(url),
                source="gpt_predicted",
                confidence_score=0.60,  # Lower confidence - algorithmic guess
                source_description="GPT-predicted based on organization name",
                notes=["Generated by AI based on naming patterns"]
            )
            candidates.append(gpt_candidate)
            logger.info(f"Added GPT-predicted URL: {gpt_candidate.url}")
    ...
    async def _get_gpt_predicted_urls(self, ein: str, organization_name: str) -> List[str]:
        """Get GPT-predicted URLs using existing discovery processor"""
        try:
            # Use existing GPT URL discovery processor
            from ..processors.analysis.gpt_url_discovery import GPTURLDiscoveryProcessor
            from ..core.data_models import ProcessorConfig, WorkflowConfig
            
            gpt_processor = GPTURLDiscoveryProcessor()
            
            # Create processor config with required fields
            workflow_config = WorkflowConfig(target_ein=ein, target_name=organization_name)
            processor_config = ProcessorConfig(
                workflow_id=f"url_resolution_{ein}",
            ...
            # Execute processor
            result = await gpt_processor.execute(processor_config)
            
            if result.success and result.data.get('urls'):
                return result.data['urls']
            else:
                logger.warning(f"GPT processor failed for {organization_name}: {result.errors}")
                return []
            
        except Exception as e:
            logger.warning(f"Failed to get GPT predictions for {organization_name}: {e}")
            return []
    ...
    def _select_primary_url(self, candidates: List[URLCandidate]) -> Optional[URLCandidate]:
        ...
        # Priority hierarchy: User > 990 > GPT (within valid candidates)
        source_priority = {
            "user_provided": 3,
            "990_declared": 2, 
            "gpt_predicted": 1
    ...
    def _generate_resolution_strategy(self, 
        ...
        source_strategies = {
            ...
            "gpt_predicted": "Using AI-predicted URL (algorithmic generation)"
        }
    ...
    def _assess_confidence(self, 
        ...
        # Risk factors
        if primary_url.source == "gpt_predicted":
            assessment["risk_factors"].append("AI-predicted URL (verify accuracy)")
    ...
    def _generate_recommendations(self, 
        ...
        elif primary_url.source == "gpt_predicted":
            recommendations.append("AI-predicted URL - verify accuracy before major operations")
