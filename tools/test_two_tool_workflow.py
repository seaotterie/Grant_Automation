...
# Add the specific tool directories to path
current_dir = os.path.dirname(os.path.abspath(__file__))
bmf_tool_dir = os.path.join(current_dir, 'bmf-filter-tool')
form990_tool_dir = os.path.join(current_dir, 'form990-analysis-tool')

sys.path.insert(0, bmf_tool_dir)
sys.path.insert(0, form990_tool_dir)
...
async def test_two_tool_workflow():
    ...
    # Extract EINs for analysis (top 5 organizations)
    target_eins = [org.ein for org in bmf_result.organizations[:5]]
    ...
    reduction_ratio = bmf_result.summary.total_in_database / len(bmf_result.organizations) if bmf_result.organizations else 0
    print(f"Filter efficiency: {reduction_ratio:,.0f}:1 reduction")

    print(f"\n🎉 Two-tool workflow test completed successfully!")

if __name__ == "__main__":
    asyncio.run(test_two_tool_workflow())